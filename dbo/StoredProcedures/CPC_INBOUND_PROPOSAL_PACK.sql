CREATE procedure [dbo].[CPC_INBOUND_PROPOSAL_PACK]
    (
    @ref_nbr varchar(50),
    @picker varchar(150)
)
...
begin
    ...
    select
        ...
        @picked_count = sum(case when a.PPC_STATUS = 'SUBMITTED' then 1 else 0 end),
        @cnt = count(a.ROW_ID)
    ...
    if (@picked_count <> @cnt)
	begin
        raiserror('Cannot pick Proposal Pack. Not all Content have been completed.', 16, 1);
        return;
    end
